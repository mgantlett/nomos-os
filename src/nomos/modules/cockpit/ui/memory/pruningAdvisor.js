...
import { cachedLessons, selectedLessonHash, renderLessonsList, setCachedLessons } from './memoryCore.js';
...
let lastAdvisorLessons = null;
let lastSelectedLessonHash = null;
...
function calculateJaccardPreTokenized(idxA, idxB, tokenizedSets) {
...
function createCollapsibleSection(titleText, count, renderContent, pruneAllFn) {
...
export function renderPruningAdvisorDashboard(container) {
    ...
    const existingWrapper = container.firstElementChild;
    ...
    const lowContextSection = createCollapsibleSection('Low-Context Candidates', lowContextCandidates.length, (body) => {
        ...
        if (consent) {
            ...
            Promise.all(lowContextCandidates.map(node => fetch('/api/memory/prune', {
                method: 'POST',
    ...
    const redundantSection = createCollapsibleSection('Redundant Memory Clusters', redundantPairs.length, (body) => {
        ...
        if (consent) {
            ...
            Promise.all(hashesToPrune.map(hash => fetch('/api/memory/prune', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
