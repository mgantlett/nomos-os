...
export function setCachedLessons(val) {
    cachedLessons = val;
}
export function setSelectedLessonHash(val) {
    selectedLessonHash = val;
}
export function setActiveTags(val) {
    activeTags = val;
}
export let activeTags = [];
export function updateTimelineUI(lessons) {
    ...
    const categories = new Set(['metadata', 'submodules', 'xss-security', 'tdd-gates', 'general', 'walkthrough', 'explainer', 'quiz']);
...
export function onMemorySearchChange(val) {
...
export function filterMemoryByCategory(category) {
...
export function filterMemoryByTag(tag) {
    activeMemoryTag = tag;
    ...
    tags.forEach(tagElement => {
        ...
        if (t === tag) {
            ...
            tagElement.style.background = 'var(--neon-blue)';
            tagElement.style.color = 'var(--text-main)';
        }
        else {
...
async function fetchSearchLessons(q, cat, tag = 'All') {
    try {
...
let selectMemoryNodeCallback = () => { };
export function registerSelectNodeCallback(fn) {
