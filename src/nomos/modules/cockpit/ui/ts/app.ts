...
// 2. Task & Kanban Boards (kanban.ts):
...
async function handlePlanApprove(btn: HTMLButtonElement): Promise<void> {
  btn.disabled = true;
...
async function handleTaskSignoff(btn: HTMLButtonElement): Promise<void> {
  btn.disabled = true;
...
async function handleGcpToggle(gcpToggleBtn: HTMLButtonElement): Promise<void> {
  ...
  try {
    ...
    showToast(`Network error toggling Cloud Substrate: ${(err as Error).message}`, 'error');
    if (errEl) {
...
import { initKanbanBoard } from './components/KanbanBoard.js';
import { resetKanbanCache, initKanbanSearch } from './board.js';

import { updateTelemetryUI, renderSteeringButtons, TelemetryStatus } from './telemetry.js';
import { renderASTGraph, ASTData } from './ast.js';
...
import { fetchActiveArtifact, saveActiveArtifact, triggerPhaseTransition, getCurrentActiveArtifactType } from './artifacts.js';
...
import { openSwarmConsoleDrawer, closeSwarmConsoleDrawer, updateSwarmDiffAndMetadata } from './swarm_drawer.js';


declare global {
  interface Window {
    refreshData: () => Promise<void>;
    openTaskDetailsDrawer: (taskId: number | string) => Promise<void>;
    openAgentDetailsDrawer: (task: any) => Promise<void>;
    closeTaskDetailsDrawer: () => void;
    toggleModal: (open: boolean) => void;
    handleDispatchSubmit: (e: Event) => Promise<void>;
    fetchActiveArtifact: (type: string) => void;
    saveActiveArtifact: () => void;
    pruneWorkspaces: (wtPath?: string) => Promise<void>;
    auditBranches: () => Promise<void>;
    pruneBranch: (branch: string) => Promise<void>;
    pruneAllBranches: () => Promise<void>;
    marked?: any;
    mermaid?: any;
  }
}
...
let latestStatus: any = null;       // Most recent /api/status response
...
let refreshTimeoutId: any = null;   // Active setTimeout reference for the refresh loop
let activeDrift: any = {};          // Current spec parity drift payload from /api/drift
let lastFleet: any = null;          // Latest fleet neighbors from /api/fleet
let lastGitbrainData: any = null;   // Latest gitbrain metrics
...
async function handleDispatchSubmit(e: Event): Promise<void> {
  e.preventDefault();
  const branch = (document.getElementById('form-branch') as HTMLInputElement).value;
  const taskId = (document.getElementById('form-task-id') as HTMLInputElement).value;
  const instruction = (document.getElementById('form-instruction') as HTMLTextAreaElement).value;
  ...
  try {
    ...
    const data = await res.json();
    if (!data.success) return showToast(`Dispatch rejected: ${data.error || 'unknown'}`, 'error');
    showToast(`Swarm Worker dispatched successfully (PID: ${data.pid})`, 'success');
    toggleModal(false);
    (document.getElementById('dispatch-form') as HTMLFormElement).reset();
    refreshData();
  } catch (err) {
...
async function handleProcessSignal(pid: number, action: string): Promise<void> {
  try {
    const res = await fetch(`/api/${action}?pid=${pid}`);
    const data = await res.json();
    if (!data.success) return showToast(`Signal delivery failed: ${data.error}`, 'error');
...
export async function fetchSafeJson<T>(url: string, defaultValue: T): Promise<T> {
  try {
    const res = await fetch(url);
    if (res.ok) {
      if (url === '/api/status') updateConnectionLED(true);
      return await res.json() as T;
    }
    if (url === '/api/status') updateConnectionLED(false);
  } catch (err: any) {
...
let lastBacklog: any[] = [];                         // Cached backlog task list
let lastAST: any = { nodes: [], links: [] };         // Cached AST dependency graph
let lastLessons: any[] = [];                         // Cached memory timeline lessons
let lastAnalytics: any = null;                       // Cached analytics dashboard data
let pollCounter = 0;                                 // Monotonic poll cycle counter
let activeSwarmsList: any[] = [];                     // Currently running swarm workers
let lastActiveSwarmsList: any[] = [];                 // Previous cycle's swarm list (for diff detection)
...
export async function refreshData(forceHttp = false): Promise<void> {
  if (refreshTimeoutId) {
  ...
  try {
    ...
    if (isWSFrame) {
      status = latestStatus;
      backlog = lastBacklog;
      swarm = (window as any).latestSwarmData || { nodes: [], links: [] };
      ...
      const fetched = await Promise.all([
        fetchSafeJson<any>('/api/status', { phaseState: { current_phase: 'IDLE', plan_approved: 'false', task_id: '' } }),
        shouldFetchLowFreq ? fetchSafeJson<any[]>(`/api/backlog?t=${Date.now()}`, []) : Promise.resolve(lastBacklog),
        fetchSafeJson<any>('/api/swarm', { nodes: [], links: [] }),
        fetchSafeJson<any[]>('/api/swarm/active-list', []),
        shouldFetchLowFreq ? fetchSafeJson<any>('/api/graph', { nodes: [], links: [] }) : Promise.resolve(lastAST),
        fetchSafeJson<any>(`/api/drift?t=${Date.now()}`, {}),
        shouldFetchLowFreq ? fetchSafeJson<any[]>(`/api/search?project=${CockpitStore.getState().activeProjectFilter}&q=&category=&tag=All&t=${Date.now()}`, []) : Promise.resolve(lastLessons),
        shouldFetchLowFreq ? fetchSafeJson<any>(`/api/analytics?t=${Date.now()}`, null) : Promise.resolve(lastAnalytics),
        fetchSafeJson<any>(`/api/gitbrain?project=${CockpitStore.getState().activeProjectFilter}&t=${Date.now()}`, null),
        fetchSafeJson<any>('/api/fleet', null)
      ]);
      status = fetched[0];
      backlog = fetched[1];
      swarm = fetched[2];
    ...
    if (CockpitStore.getState().activeProjectFilter !== 'ALL' && !(status && status.edition === 'community')) {
      activeSwarms = (activeSwarms || []).filter((sw: any) => sw.path?.toLowerCase().includes(CockpitStore.getState().activeProjectFilter.toLowerCase()));
    }
    ...
    (window as any).activeSwarmsList = activeSwarms;

    latestStatus = status;
    (window as any).latestStatus = latestStatus;
    (window as any).lastBacklog = lastBacklog;
    (window as any).latestSwarmData = swarm;
    ...
    if (status && status.version) {
      ...
      if (vEl) vEl.textContent = status.version;
    }
    ...
    updateTelemetryUI(status as TelemetryStatus);
    ...
    (window as any).swarmSlotsData = status.slots;

    updateHeaderTelemetryUI(status);

    // Update LLM LEDs in Header
    (status.inferenceStats || []).forEach((daemon: any) => {
    ...
    if (status.gpu) {
      if (gpuUtilEl) gpuUtilEl.textContent = status.gpu.gpuUtil;
      if (vramValEl) vramValEl.textContent = status.gpu.vramUsed;
    ...
    if (tpsValEl) tpsValEl.textContent = `${(win.maxTotalTps || 0).toFixed(1)}`;

    const activeTaskId = status.phaseState?.task_id;
    const ideActiveTaskId = status.idePhaseState?.task_id || '';
    (window as any).ideActiveTaskId = ideActiveTaskId;
    ...
    if (selectEl) {
      const walkthroughOpt = selectEl.querySelector('option[value="walkthrough"]') as HTMLOptionElement | null;
      if (walkthroughOpt) {
    ...
    if (activeTab === 'tab-overview' || activeTab === 'tab-ast') {
      renderASTGraph(ast as ASTData, activeDrift?.modifiedFiles || []);
    }
    ...
    if (!(window as any)._hasInitWs && status.edition === 'sovereign') {
      (window as any)._hasInitWs = true;
...
window.addEventListener('load', () => {
  ...
  const themeSwitcher = document.getElementById('ui-theme-switcher') as HTMLSelectElement | null;
  if (themeSwitcher) {
  ...
  initHeader();
  initKanbanBoard();
  // initControlPlaneWS is now called after the first /api/status fetch completes.

  addWSListener((frame: any) => {
    if (frame.type === 'status') {
      latestStatus = frame.payload;
      (window as any).latestSwarmData = latestStatus.swarm || { nodes: [] };
      ...
      if (CockpitStore.getState().activeProjectFilter !== 'ALL' && !(latestStatus && latestStatus.edition === 'community')) {
        activeSwarmsList = activeSwarmsList.filter((sw: any) => sw.path?.toLowerCase().includes(CockpitStore.getState().activeProjectFilter.toLowerCase()));
      }
      (window as any).activeSwarmsList = activeSwarmsList;
      isWSFrame = true;
      refreshData();
  ...
  const registerDrawerResizer = (splitterId: string, drawerId: string, storageKey: string, defaultWidth: string) => {
  ...
  const mermaidObj = window.mermaid || (window as any).mermaid;
  if (mermaidObj) {
  ...
  const syncCheckbox = document.getElementById('sync-scroll-chk') as HTMLInputElement | null;
  ...
  const kanbanStateToggle = document.getElementById('kanban-state-toggle') as HTMLInputElement | null;
  ...
  const matrixSort = document.getElementById('matrix-sort') as HTMLSelectElement | null;
  if (matrixSort) {
  ...
  const gcpToggleBtn = document.getElementById('gcp-spot-toggle-btn') as HTMLButtonElement | null;
  if (gcpToggleBtn) {
 ...
 */
export function switchBottomTab(targetTab: string): void {
  const btn = document.querySelector(`.tab-btn[data-tab="${targetTab}"]`) as HTMLButtonElement | null;
  if (btn) {
    btn.click();
  }
}
 ...
 */
function clearChatHistoryFromStorage(): void {
  [localStorage, sessionStorage].forEach(s => {
    for (let i = s.length - 1; i >= 0; i--) {
      const k = s.key(i);
      if (k && (k.includes('chat') || k.includes('history'))) s.removeItem(k);
    }
  });
...
function updateWorktreesUI(worktrees: any[]): void {
 ...
 */
function createApproveButton(background: string, label: string, onClick: (btn: HTMLButtonElement) => Promise<void>): HTMLButtonElement {
...
function updateHITLBanner(status: any): void {
  ...
  if (!banner || !msgEl || !actionsEl) return;

  const currentPhase = status.phaseState?.current_phase;
  const planApproved = status.phaseState?.plan_approved;
  const taskId = status.phaseState?.task_id || (window as any).ideActiveTaskId || '';
  ...
  if (currentPhase === 'PLAN' && planApproved !== 'true') {
    ...
    viewBtn.onclick = () => {
      ...
      if (specTabBtn) (specTabBtn as HTMLElement).click();
      const leftTabBtn = document.querySelector('[onclick*="switchTab(\'tab-spec-parity\'"]') as HTMLElement;
      if (leftTabBtn) leftTabBtn.click();
    };
    ...
    if (!isIDE) {
      const approveBtn = createApproveButton(
        ...
        '✅ Approve Plan & Enter EDIT',
        async (btn) => { await handlePlanApprove(btn); }
      );
    ...
    if (!isIDE) {
      const approveBtn = createApproveButton(
        ...
        '🚀 Approve & Commit/Push',
        async (btn) => { await handleTaskSignoff(btn); }
      );
      actionsEl.appendChild(approveBtn);
    }
  } else {
...
function renderSlotStateNode(slotState: any, listEl: HTMLElement): void {
...
async function handleDaemonToggle(daemonName: string, isOnline: boolean, btn: HTMLButtonElement): Promise<void> {
  btn.disabled = true;
  const action = isOnline ? 'stop' : 'start';
...
function renderDaemonBadge(daemon: any, daemonContainer: HTMLElement): void {
...
function detectStartedTaskWorkers(activeSwarms: any[]): void {
  activeSwarms.forEach((sw: any) => {
    if (!sw.id || sw.status !== 'running') return;
    const wasRunning = lastActiveSwarmsList.some((p: any) => p.id === sw.id && p.status === 'running');
 ...
 */
async function fetchInspectData(path: string): Promise<any> {
  try {
    const data = await fetchSafeJson<any>(`/api/swarm/inspect?path=${encodeURIComponent(path)}`, null);
    if (data && data.phaseState && data.drift) return data;
  } catch (err) {
 ...
 */
function updateSlotsUI(slots: any): void {
  const listEl = document.getElementById('slots-list-container');
  if (!listEl || !slots) return;
  ...
  if (slots.slotStates && slots.slotStates.length > 0) {
    slots.slotStates.forEach((slotState: any) => renderSlotStateNode(slotState, listEl));
  } else {
...
(window as any).changeLogSource = changeLogSource;
(window as any).openSwarmConsoleDrawer = openSwarmConsoleDrawer;
(window as any).closeSwarmConsoleDrawer = closeSwarmConsoleDrawer;

// Expose methods globally for callback routes
(window as any).CockpitStore = CockpitStore;
(window as any).refreshData = refreshData;

(window as any).handleHeaderDaemonToggle = async (daemonName: string, ledEl: HTMLElement) => {
  if (ledEl.style.opacity === '0.5') return; // Debounce
...
(window as any).pruneWorkspaces = pruneWorkspaces;
(window as any).auditBranches = auditBranches;
(window as any).pruneBranch = pruneBranch;
(window as any).pruneAllBranches = pruneAllBranches;
(window as any).switchBottomTab = switchBottomTab;
(window as any).updateHeaderContextIndicator = updateHeaderContextIndicator;

(window as any).openSwarmChatDrawer = openSwarmChatDrawer;
(window as any).closeSwarmChatDrawer = closeSwarmChatDrawer;
(window as any).sendUserChatMessage = sendUserChatMessage;

// GitBrain Memory Bindings
(window as any).onMemorySearchChange = onMemorySearchChange;
(window as any).filterMemoryByCategory = filterMemoryByCategory;
(window as any).filterMemoryByTag = filterMemoryByTag;
(window as any).lastBacklog = lastBacklog;
(window as any).latestStatus = latestStatus;

// Page Visibility API optimization for Task 325 GPU Power Draw
document.addEventListener('DOMContentLoaded', async () => {
    // Dynamically load Sovereign plugins if edition is sovereign
    // We defer this slightly so the first /api/status fetch can populate latestStatus
    setTimeout(async () => {
        const isSovereign = (window as any).latestStatus?.edition === 'sovereign';
        if (isSovereign) {
            console.log('Sovereign edition detected. Loading proprietary UI plugins...');
            try {
                // @ts-ignore
                await import('/plugins/sovereign.js');
            } catch (e) {
                console.error('Failed to load Sovereign plugin:', e);
            }
        }
    }, 1000);
});
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    (window as any).__isWebUiTabHidden = true;
  } else {
    (window as any).__isWebUiTabHidden = false;
    refreshData();
  }
});

function showHITLAlertBanner(taskId: string, msg: string) {
  const banner = document.getElementById('hitl-alert-banner');
...
(window as any).showHITLAlertBanner = showHITLAlertBanner;
