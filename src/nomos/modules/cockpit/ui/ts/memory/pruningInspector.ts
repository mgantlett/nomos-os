...
export function selectMemoryNode(hash: string): void {
  ...
  const uniqueCats = new Set<string>(['metadata', 'submodules', 'xss-security', 'tdd-gates', 'general', 'walkthrough', 'explainer', 'quiz']);
  cachedLessons.forEach(l => {
    if (l.category && l.category.trim()) uniqueCats.add(l.category.trim());
  });
  ...
  if (node.category !== 'quiz') {
    ...
    if ((window as any).marked) {
      mdView.innerHTML = (window as any).marked.parse(node.insight || '');
    } else {
