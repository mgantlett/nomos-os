...
let cachedDebtData: any = null;
let lastDebtFetchTime = 0;

export function renderTechnicalDebtRegistry(listContainer: HTMLDivElement): void {
  ...
  fetch('/api/quality-debt')
    .then(res => {
      ...
      return res.json() as Promise<{ success: boolean; active_debt?: any[]; error?: string }>;
    })
    .then(data => {
      cachedDebtData = data;
      renderData(data);
